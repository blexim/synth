...
complete procedure for the termination of fixed-width and floating-point
arithmetic programs.
%
\end{abstract}
...
Program Synthesis, Lexicographic Ranking Functions, Bitvector Ranking Functions,
...
complete for deterministic bit-vector programs -- for any program, we will find
either a proof of its termination or non-termination.  For non-deterministic
programs, we are only semi-complete -- if the program terminates, we will find a proof,
but if it does not terminate we may fail to discover a recurrence set.
...
properties of all of the loops in the program simultaneously, including
nested loops.
...
{\bf Limitations.}
Our algorithm proves termination for transition systems with finite state
...
rich enough to analyse code which uses the heap or arrays.  Also, we have not
yet added support for recursion to our encoding.
...
To make solutions of our constraints tractable, we identify a fragment of second-order logic
with a constrained use of quantification that is expressive enough to encode
both termination and non-termination, while admitting a semi-decision procedure.
...
For the time being, a loop $L(G, T)$ is defined by a predicate $G$ and relation $T$.
G states $x$ satisfying the loop's guard are given by the
...
To show that $L(G, T)$ is unconditionally terminating (i.e. it eventually terminates regardless of the
state it starts in), it suffices to find a ranking function for \mbox{$T \cap (G \times X)$},
...
well-foundedness.  In contrast, our formulation in Definition~\ref{def:UT}
...
induces a infinite execution iff some state is visited infinitely
often, or equivalently $ \exists x . T^+(x, x)$.
Deciding satisfiability of this formula directly would require a logic
that includes a transitive closure operator, $T^+$.  Rather than
...
The notion of open recurrence set is encoded by formula {\bf [ONT]} in Definition~\ref{def:ont}.
...
In order to express the non-termination problem inside the synthesis
...
of formula {\bf [CNT]} in Definition~\ref{def:cnt}, which is in the synthesis
fragment.
...
and hence we cannot specify a proof of its non-termination
with formula {\bf [CNT]}.
...
from the specification.  We ran the resulting termination prover on 47
benchmarks taken from the literature and SV-COMP'15~\cite{svcomp}.  To
...
find a termination proof.  All of these cases have nested loops, which are
easy for us to handle, but can be difficult for provers that enumerate
lassos.
...
Given the lack of research in handling not-linear programs, as well as
...
tool~\cite{DBLP:conf/cav/BrockschmidtCF13}. 
In~\cite{DBLP:conf/cav/BradleyMS05}, Bradley et al.~combine the generation
of ranking functions with the generation of invariants to form a single
constraint solving problem such that the necessary supporting invariants for
the ranking function are discovered on demand.  In our setting, both the
ranking function and the supporting invariant are iteratively constructed in
the same refinement loop.
...
feasibility~\cite{DBLP:conf/popl/GuptaHMRX08}. 
In~\cite{DBLP:conf/tacas/ChenCFNO14}, Chen et al.~prove nontermination via
reduction to safety proving.  Their iterative algorithm uses counterexamples
to a fixed safety property to refine an under-approximation of a program.
