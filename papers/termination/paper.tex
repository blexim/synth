...
Moreover, most works on termination assume restrictive transition relations and are concerned with linear ranking functions. However, the probability of a random program having a linear ranking function is very low ? (see Section ?). 


We propose a general framework that does not assume the existence of linear ranking functions and can uniformly handle:
\begin{itemize}
...
For this purpose, we rephrase the termination problem as a synthesis problem.
 As a direct consequence, the complexity of the method is dependent on the Kolmogorov complexity of the computed ranking function (+ it's not dependent on the size/number of variables of the analysed program). We empirically observe that most programs have ranking functions with low Kolmogorov complexity (see Section ?). 
