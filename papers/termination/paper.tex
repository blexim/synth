...
\label{def:conditional-termination-formula}
 \begin{align*}
  \exists R, Inv . \forall \mathcal{V}, \mathcal{V'} . & I(\mathcal{V}) \wedge G(\mathcal{V}) \rightarrow Inv(\mathcal{V}) ~ \wedge \\
                                 & G(\mathcal{V}) \wedge V(\mathcal{V}) \wedge T(\mathcal{V}, \mathcal{V'}) \rightarrow Inv(\mathcal{V'}) \wedge R(\mathcal{V}) > 0 
...
\noindent{\bf Generalised synthesis formula.}
...
\emph{lassos} in the program's CFG. %(they  make use of the syntactic notion of lasso).
...
Our method is purely semantic. %We extend the notion of recurrence set in order to formulate the 
We formulate the non-termination problem at the loop level.%, rather than transition relation.
In our design, a loop $L=(\mathcal{V},I,G,t)$ is non-terminating iff it has a recurrence set of states.
