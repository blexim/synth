...
As this definition of ranking function is very general, research is often limited to some
...
In doing so, an analysis ensures that such a ranking function will be found is one  exists, 
...
Next, we will look in more detail at the implications of the assumptions made by such techniques.\\
...
frequently in low-level code and are not handled properly by existing techniques. 
...
although a linear ranking function exists (see Figure~\ref{fig:handletable}). 
...
Figure~\ref{} is an example of a linear program that only admits lexicografic non-linear ranking functions.
...
We have collected a number of motivational examples from other termination papers that treated bit-vectors as mathematical integers \cite{DBLP:conf/tacas/LeikeH14,DBLP:conf/tacas/CookSZ13}. 
