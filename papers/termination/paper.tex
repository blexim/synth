...
Let us assume that we have already proven the termination of loops 1 and 3, and we are now trying to prove that loop 2 terminates. The 
termination of loop 2 is encoded as the conjunction of constraints on the right of Figure~\ref{fig:environment-model}:
...
Dual to termination, when attempting to prove non-termination, we must under-approximate the environment.
Consider the example in Figure~\ref{fig:nonterm-environment}.  We are attempting to prove non-termination
of the innermost loop, which we do by finding a recurrence set $N_4$.  We will assume that the other two
loops iterate only a finite number of times.  If the constraint system is satisfiable, $x_0$ is some initial
state in which the program begins executing.  Then $N_1$ is an inductive invariant of $L_1$ that is strong
...
re-establish $N_2$ when $L_3$ exits.  Since we assumed that $L_2$ iterated only a finite number of times,
...
\begin{minipage}{0.82\textwidth}
\begin{align*}
 \exists N_1, N_2, N_3, N_4, x_0 . \forall x, x' . \\
  N_1(x_0) \, \wedge \\
  c_1(x) \wedge N_1(x) \wedge B_1(x, x') & \rightarrow N_1(x') \, \wedge \\
  \lnot c_1(x) \wedge N_1(x) & \rightarrow N_2(x) \, \wedge \\
  N_2(x) & \rightarrow c_2(x) \, \wedge \\
  c_2(x) \wedge N_2(x) \wedge P(x, x') & \rightarrow N_3(x') \vee N_4(x') \, \wedge \\
  c_3(x) \wedge N_3(x) \wedge B_3(x, x') & \rightarrow N_3(x') \, \wedge \\
  \lnot c_3(x) \wedge N_3(x) & \rightarrow N_2(x) \, \wedge \\
  N_4(x) & \rightarrow c_3(x) \, \wedge \\
  N_4(x) \wedge B_3(x, x') & \rightarrow N_4(x')
\end{align*}
\end{minipage}
\end{framed}
