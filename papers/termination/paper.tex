...
\begin{theorem}
 \label{thm:snt}
 Formula {\bf [ONT]} (Definition~\ref{def:ont}) is satisfiable iff {\bf [SNT]} (Definition~\ref{def:snt}) is.
\end{theorem}
...
A very inefficient construction which computes the first co-ordinate
of the output $y$ is:
%
\begin{verbatim}
t1 = f(0)
...
\begin{proof}
 ...
 in $\mathcal{L}$.  Since $\phi$ is a termination specification, $P_T \models \phi$ and so
 ...
 proof $P_N$ expressible in $\mathcal{L}$, so $P_N \models \psi$ and hence for any $P_T$, $(P_T, P_N) \models \phi \vee \psi$.
 
 So in either case ($L$ terminates or does not), there is a witness in $\mathcal{L}$ satisfying $\phi \vee \psi$.
\end{proof}
...
\begin{proof}
 The background logic for Algorithm~\ref{alg:termination} is \newC, for which safety is decidable.  Also, the set of
 inputs is just the state space of $L$, which is finite.  Therefore Theorems~\ref{thm:generalised-sat} and \ref{thm:synth-semicomplete}
 hold -- the specification \lstinline!spec! is satisfiable and the instanciation of Algorithm~\ref{alg:cegis} is semi-complete.
 Therefore the call to {\sc Synthesise} terminates with a witness $(P_T, P_N)$.
