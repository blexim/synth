...
This explain why approaches based on Ramsey's theorem are restricted to searching for linear termination arguments.
...
For this purpose, we phrase the termination problem as a second order satisfaction problem and
we identify a fragment of second-order logic that is expressive enough to capture our formulation %several variations of the halting problem 
(Section~\ref{sec:second.order}), which we call \emph{the synthesis fragment}. We propose a method 
...
exist, it will be found. With respect to its performance, we show that its runtime is dominated by the size of 
...
This means that our procedure is not directly dependent on the size of the analysed program or its number of variables, but on the Kolmogorov
complexity of its minimal termination argument. We argue that it is natural to expect programmers to write programs that have relatively short termination 
proofs as this will make them understandable.


%% Our approach to proving termination has two distinct features over previous work. 
%% First,
%% Second, our runtime is dependant on the KC of the shortest termination proof rather than the structure of the loop, number of variables, number of lassos... 
...
This is opposed to tools that alternate 
...
In our setting, both the ranking function and the supporting invariant are iteratively constructed through a single refinement loop.
...
Now, we provide some simple examples that break those assumptions and cannot therefore be handled by state of the art tools.
...
A program not falling in this category is given in Figure~\ref{fig:motivation.a}.
...
the semantics of the bit-wise AND operator, which is not easily done when working with mathematical integers.
...
The program in Figure~\ref{fig:motivation.b} breaks a different assumption made by the majority of termination analyses, namely that \emph{ranking functions are linear}.
Our technique proves that this loop terminates by finding that $|x|$ is a possible ranking function for it.

Figure~\ref{fig:motivation.c} invalidates the assumption that \emph{integer semantics and bit-vector semantics are equivalent}. 
This loop is  terminating for bit-vectors since \texttt{x}
will eventually over-flow and become negative. Conversely, the same program is non-terminating using integer
arithmetic since the loop condition stays always true for any initial \texttt{x} at least 1.

The same assumption is broken by the program in Figure~\ref{fig:motivation.d}. Every execution of the program can be partitioned into two phases: in the first phase $y$ increases
...
The loops in Figure~\ref{fig:motivation.e} and Figure~\ref{fig:motivation.f} break the assumption that \emph{real semantics and float semantics are equivalent}. 
% ignore potential rounding errors, under- and over-flows, which are precisely what makes  reasoning about floating point inherently difficult.
This assumption may lead to erroneous diagnosis of a program's terminating behaviour.% as illustrated by the loops in Figure~\ref{} and Figure~\ref{}.
While the former does not terminate for reals, but does for floats, the latter 
terminates for reals, but does not for floats.


Figure~\ref{fig:motivation.g} is a linear program taken from \cite{DBLP:conf/tacas/CookSZ13}, where it is shown to not admit (without prior manipulation) a lexicographic linear function, but only a disjunctively well-founded one, which requires an expensive binary reachability analysis.
...
The program in Figure~\ref{fig:motivation.i} is taken from SVCOMP'15  \footnote{http://sv-comp.sosy-lab.org/2015/index.php} termination benchmarks.
...
Given a transition system, %with state space $X$ and transition relation $T \subseteq X \times X$, 
...
$\vec{x_1}, \vec{x_2}, \ldots$ with $\forall i . T(\vec{x_i}, \vec{x_{i+1}})$.  
\end{definition}

We can prove that $T$ is
unconditionally terminating by finding a ranking function.

\begin{definition}[Ranking function]
Given a transition system with state space $X$ and transition relation $T \subseteq X \times X$,
a ranking function is an injective function $R: X \to Y$ where
$Y$ is well-founded and $R$ is monotonically decreasing with respect to $T$.  That is
to say:
$$\forall \vec{x}, \vec{x'} . T(\vec{x}, \vec{x'}) \Rightarrow R(\vec{x}) < R(\vec{x'})$$
\end{definition}
...
logic.  In fact, many of these problems can be encoded in a fragment of second-order logic we will
suggestively refer to as the \emph{synthesis fragment}:
...
\begin{definition}[Synthesis Fragment]
 ...
 Here $P$ ranges over sets, while $x$ and $y$ range over ground terms.
\end{definition}

We will then show that checking satisfiability of a formula in the synthesis fragment corresponds to program synthesis.
We will make use of this observation to frame the halting problem as a program synthesis problem, allowing us
to employ the program synthesiser described in Section~\ref{sec:synthesis}.

\subsection{Variations of the Halting Problem}
...
\begin{align*}
 \exists R . \forall x, x' . & L(x) \wedge T(x, x') \rightarrow R(x) > 0 \wedge R(x) > R(x')
\end{align*}
\end{definition}
...
\label{def:conditional-termination-formula}
 \begin{align*}
  \exists R, V . \forall x, x' . & I(x) \wedge L(x) \rightarrow V(x) ~ \wedge \\
...
\label{def:multi-termination-formula}
 \begin{align*}
  \exists R, V_1, V_2 . \forall x, x' . & I(x) \rightarrow V_1(x) ~ \wedge \\
                                        & V_1(x) \wedge L_1(x) \wedge T(x, x') \rightarrow V_1(x') ~ \wedge \\
                                        & V_1(x) \wedge \lnot L_1(x) \wedge L_2(x) \rightarrow V_2(x) ~ \wedge \\
...
\label{def:nonterm-formula}
 \begin{align*}
  \exists N, x_0 . \forall x . \exists x' . & I(x_0) \wedge L(x_0) \wedge N(x_0) ~ \wedge \\
...
\label{def:deterministic-nonterm-formula}
 \begin{align*}
  \exists N, x_0 . \forall x, x' . & I(x_0) \wedge L(x_0) \wedge N(x_0) ~ \wedge \\
...
$R$ is a ranking function that proves loop $L$ terminates.
...
$V$ is an inductive invariant of $L$ that is established by the initial states $I$ if the loop
condition is met.
...
for all $y$, we have found a solution to the synthesis problem and write $P \models \sigma$.
...
\begin{figure*}
\centering
\begin{tabular}{|l|r|c|c|c|c|c|r|r|c|r|}
\hline
    & LOC & Terminates? & \shortstack{Linear \\ program?} & \shortstack{Linear ranking \\ function?}  & Conditional? & Float? & Dimension & \shortstack{Ranking \\ program size} & Res & Time (s)\\
    \hline
    \hline
\input{table}
    \hline
