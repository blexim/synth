...
We represent programs as transition systems with state space $X$, transition relation $T \subseteq X \times X$, and set of variables $\mathcal{V}$. 
A state of the program is a valuation of the variables from $\mathcal{V}$.

\begin{definition}[Unconditional termination]
A transition system is said to be \emph{unconditionally terminating} if there is no infinite sequence
$x_1, x_2, \ldots$ with $\forall i . T(x_i, x_{i+1})$.  
\end{definition}

We can prove that $T$ is unconditionally terminating by finding a ranking function.

\begin{definition}[Ranking function]
An injective function ${R:X\to Y}$ is a ranking function if $Y$ is well-founded and 
...
A linear ranking function $R: X \to Y$ 
...
For $Y = Z^m$, we say that a ranking function $R: X \to Y$ is lexicographic
...
\begin{definition}{Conditional termination}
\end{definition}
...
\end{lstlisting}
 ...
 In fact, many of these problems can be encoded in a fragment of second-order logic we will
...
\label{def:conditional-termination-formula}
 \begin{align*}
  \exists R, V . \forall x, x' . & I(x) \wedge G(x) \rightarrow V(x) ~ \wedge \\
                                 & G(x) \wedge V(x) \wedge T(x, x') \rightarrow V(x') \wedge R(x) > 0 \wedge R(x) > R(x')
 \end{align*}
\end{definition}
...
We first consider a single loop $L = (I,G,T)$ consisting of the initial states $I : X \rightarrow B$, the loop guard $G : X \rightarrow B $ and a body $T : X \rightarrow X$.
The existence of a ranking function for this loop is equivalent to the satisfiability
...
Gupta et al. \cite{DBLP:conf/popl/GuptaHMRX08} characterize non-termination of a transition relation $T$ by the existence of a
\emph{recurrence set} (open recurrence set in the terminology of \cite{DBLP:conf/tacas/ChenCFNO14}), 
i.e. a nonempty set of states $G$ such that for each $s \in G$ there
exists a transition to some $s'\in G$. 
For our case, given a loop $L = (I,G,T)$, the notion of open recurrent set is encoded by Definition~\ref{def:nonterm-formula}.
...
In order to formulate the non-termination problem in the synthesis fragment, we make use of the notion of \emph{closed recurrence set} introduced by Chen et al. 
in \cite{DBLP:conf/tacas/ChenCFNO14}. As ooposed to a closed recurrence set, this is a  purely universal property: for each $s \in G$, each of its successors 
must be in the recurrence set. The non-termination problem thus is equivalent to the
satisfiability of Definition~\ref{def:deterministic-nonterm-formula}.


\cite{DBLP:conf/popl/GuptaHMRX08,DBLP:conf/tacas/ChenCFNO14} make use of the syntactic notion of \emph{lasso}.
%The standard way of proving non-termination is by detecting non-terminating lassos \cite{DBLP:conf/popl/GuptaHMRX08,DBLP:conf/tacas/ChenCFNO14}.
...
location by following a cyclic path through the controlflow
graph. A lasso induces an infinite execution if the infinite path obtained by traversing the stem and then unrolling
...
In contrast, our method is purely semantic ...
