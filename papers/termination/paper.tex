...
Next, we show how these assumptions are broken by even simple programs that can't therefore be handled accurately by state of the art termination tools.
...
where it is shown to not admit (without prior manipulation) a lexicographic linear function, but only a disjunctively well-founded one, which requires an expensive binary 
...
We represent programs as transition systems with state space $X$, transition relation $T \subseteq X \times X$
...
with $\dim(X) = n$ and $\dim(Y) = m$ is of the form:
is one that can be expressed as an $n \times m$ matrix $M$:
$$f(\vec{x}) = M\vec{x}$$
\end{definition}

\begin{definition}[Lexicographic ranking function]
If $Y = Z^m$, we say that a ranking function $R: X \to Y$ is lexicographic
and maps each state in $X$ to a tuple of values such that the loop transition leads to a decrease with
