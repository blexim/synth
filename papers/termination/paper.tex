...
This proof style proves that a transition relation is disjunctively well founded by finding a set of simple
functions, each of which proves well foundedness for part of the transition relation.
...
there are cases in which proofs based on local measures involve linear functions while corresponding global
measures require non-linear functions or lexicographic orders. 
%This explains why approaches based on Ramsey's theorem are restricted to searching for linear termination arguments.


One difficulty with a Ramsey-based approach is that validity of the termination argument can only be checked by considering the \emph{transitive closure}
of the program, rather than a single step.
...
there is a tradeoff between the complexity of the termination arguments and that of checking their validity.
...
This means that our ranking functions may involve non-linearity and lexicographic orders (we do not commit to any particular syntactic form, and do not use templates).
...
that bit-vector semantics and integer semantics give rise to the same termination behaviour. Thus, most of the techniques treat
bit-vectors and floats as mathematical integers and reals respectively 
\cite{DBLP:conf/pldi/CookPR06,DBLP:conf/popl/Ben-AmramG13,DBLP:conf/vmcai/P04,DBLP:conf/atva/HeizmannHLP13,DBLP:conf/vmcai/BradleyMS05,DBLP:conf/cav/KroeningSTW10}. 


By assuming bit-vector semantics to be identical to integer semantics,
these techniques ignore the wrap-around behaviour caused by overflows,
which can be unsound.
In Section~\ref{sec:motivation}, we show that integers and bit-vectors exhibit
incomparable behaviours with respect to termination, i.e.
programs that terminate for integers may \emph{not} terminate for bit-vectors and
vice-versa.
Thus, abstracting bit-vectors to integers may give rise to 
...
uniformly computes
lexicographic, non-linear 
ranking functions supported by non-linear inductive invariants for loops with 
arbitrary guards and transitions over bit-vectors and floats.
It also computes non-linear recurrence sets, which comprise non-termination proofs.
Our encoding uniformly multiple loops and nested loops, without having to enumerate
lassos.
...
For this purpose, we phrase the termination and non-termination problems as second order satisfaction problems.
We identify a fragment of second-order logic that is expressive enough to capture our formulation,
which we call the \emph{synthesis fragment}. Subsequently, we propose a method
for checking the satisfiability of a formula in the synthesis fragment by solving an isomorphic program synthesis problem (Section~\ref{sec:synthesis}).
Our method is sound and semi-complete, i.e. if a pair of a ranking function and a supporting invariant 
exist it will be found, but if the program does not terminate we may not find a recurrence set.  However
if the program's transition relation is deterministic, our method is complete.
...
%\item We propose a non-trivial formulation of the (non-)termination problem inside a second order logic fragment with only one quantifier alternation.
...
Figure~\ref{fig:handletable} illustrates the most common assumptions made by existing termination analyses:
\begin{itemize}
...
\item Program (h) illustrates conditional termination.
When proving program termination we are simultaneously solving two problems:
the search for a termination argument, and the search for a supporting invariant~\cite{DBLP:conf/cav/BrockschmidtCF13}.
For this loop, we find the ranking function $R(x) = x$ together with the supporting invariant $y=1$.


\item In the terminology of \cite{DBLP:conf/tacas/LeikeH14}, program (i) admits a multiphase ranking function computed from a multiphase ranking template.
...
In our setting this type of programs does not need special treatment, as we can find a non-linear lexicographic ranking function
$R(x, y, z) = (x < y, z)$.  This termination argument is somewhat subtle, but can be understood as follows: the boolean quantity $x < y$
eventually decreases, that is to say once a state with $x \geq y$ is reached, $x$ never again becomes greater than $y$.  This means
that as soon as the ``else'' branch of the if statement is taken, it will continue to be taken in each subsequent iteration of the loop.
Meanwhile, if $x < y$ has not decreased (i.e. we have stayed in the same branch of the ``if'') then $z$ does decrease.  Since
a boolean only has two possible values, it cannot decrease indefinitely.  Since $z > 0$ is a conjunct of the loop guard,
$z$ cannot decrease indefinitely, and so $R$ proves that the loop is well founded.

As with all of the termination proofs presented in this paper, the ranking function above was found completely automatically.
...
\noindent {\bf Non-determinism.} A non-termination diagnosis procedure based on checking the satisfiability of formula {\bf [CNT]}
...
For a loop $L=(G,T)$, the formulae {\bf [CT]} and {\bf [CNT]} from Definition~\ref{def:ct} and Definition~\ref{def:cnt} are in terms of 
...
All we need to do in order to use {\bf [CT]} and {\bf [CNT]} in any context %environment and regardless of the complexity of the loops's body 
is generate constraints abstracting 
...
we will not provide the full algorithm, but rather illustrate it through examples.\\
...
%
The example in Figure~\ref{fig:environment-model} contains three loops. %, which we will number 1 to 3 according to their program order.
Let us assume that we have already proven the termination of $L_1$ and $L_3$, and we are now trying to prove that $L_2$ terminates, which we encode
...
%the lines marked as $*$ over-approximate the states in which loop 2 can start execute.
%% contains a non-trivial program
...
Dual to termination, when attempting to prove non-termination, we must under-approximate the environment and the loop body's and apply 
formula {\bf [CNT]}. However, there is one more constraint that we must obey when doing so: we must stay inside the synthesis fragment! 
This can be achieved for the environment as its under-approximation $I$ and the loop under consideration are at the same level of nestedness and do not therefore 
require another quantifier alternation. 

However, when dealing with nested loops, computing the under-approximation of the loop's body 
requires an extra quantifier alternation. The origin of the problem comes from the fact that an under-approximation is an existential property and therefore
requires an extra existential quantifier, resulting in the $\exists \forall \exists$ alternation. This situation is similar to the one encountered when proving non-termination of non-deterministic loops. 
...
%% The issue appears when we try to under-approximate the body of the loop. 
%% Here we encounter a similar problem with the one for non-determinism mentioned above. 

%% Consider the generic example in Figure~\ref{fig:nonterm-environment}, where we are attempting to prove non-termination
%% of the innermost loop $L_3$ by finding a closed recurrence set $N_3$. 
%% -- We will assume that the other two loops iterate only a finite number of times.  

%% We now explain the constraints on the right of the figure.
%% Given that we prove non-termination, we try to  under-approximate the states from which $L_3$ can start executing. 
%% We do this in several steps:

%% $W_1$, $W_2$ and $W_3$ are inductive invariants corresponding to $L_1$, $L_2$ and $L_3$, respectively, such that:

%% - $W_1$ is an inductive invariant of $L_1$ that is strong enough to establish $W_2$ if $L_1$ exits (which it does by assumption).
%% - $W_2$ is an inductive invariant of $L_2$ that is strong enough to establish either $W_3$ or $N_3$ on entry to $L_3$, depending on whether $L_3$ terminates or not.
%% - Recall that $N_3$ is a recurrence set for $L_3$, so if a state $x \in N_3$ is reached, the loop $L_3$ will execute forever. Otherwise, $W_3$ holds on entry to $L_3$.
%% - $W_3$ is an inductive invariant of $L_3$ that is strong enough to re-establish $W_2$ when $L_3$ exits.


%% If the non-termination formula is satisfiable, $x_0$ is some initial state in which the program begins executing.  



%% First, we under-approximate the states in which $L_2$ can start executing through $W_2$.
%% Then $N_1$ is an inductive invariant of $L_1$ that is strong
%% enough to establish $N_2$ if $L_1$ exits (which it does by assumption).  Now, $N_2$ is an inductive invariant
%% of $L_2$ that is strong enough to establish either $N_3$ or $N_4$ on entry to $L_3$.  Recall that $N_4$ is
%% a recurrence set for $L_3$, so if a state $x \in N_4$ is reached, the loop $L_3$ will execute forever.
%% Otherwise, $N_3$ holds on entry to $L_3$.  $N_3$ is an inductive invariant of $L_3$ that is strong enough to
%% re-establish $N_2$ when $L_3$ exits.


  

%% If the constraint system is satisfiable, $x_0$ is some initial
%% state in which the program begins executing.  


%%   Since we assumed that $L_2$ iterated only a finite number of times,
%% $N_3$ must be established only a finite number of times, which means that $N_4$ will eventually be reached,
%% at which point $L_3$ will loop forever since $N_4$ is a recurrence set for $L_3$.
...
%% \begin{align*}
%%  \exists W_1, W_2, W_3, N_3, x_0 . \forall x, x' . \\
%%   W_1(x_0) \, \wedge \\
%%   c_1(x) \wedge W_1(x) \wedge B_1(x, x') & \rightarrow W_1(x') \, \wedge \\
%%   \lnot c_1(x) \wedge W_1(x) & \rightarrow W_2(x) \, \wedge \\
%%   W_2(x) & \rightarrow c_2(x) \, \wedge \\
%%   W_2(x) \wedge P(x, x') & \rightarrow W_3(x') \vee N_3(x') \, \wedge \\
%%   c_3(x) \wedge W_3(x) \wedge B_3(x, x') & \rightarrow W_3(x') \, \wedge \\
%%   \lnot c_3(x) \wedge W_3(x) & \rightarrow W_2(x) \, \wedge \\
%%   N_3(x) & \rightarrow c_3(x) \, \wedge \\
%%   N_3(x) \wedge B_3(x, x') & \rightarrow N_3(x')
%% \end{align*}
%% \end{minipage}
%% \end{framed}

%% \caption{The constraint system for non-termination\label{fig:nonterm-environment}}
%% \end{figure*}
...
We begin with a population of random programs,
then iteratively evolve this population by applying the genetic operators
{\sc crossover} and {\sc mutate}. The {\sc crossover} operator takes two programs
and combines their code in some way to create a new program.  The {\sc mutate} operator takes a single
...
proportion to their observed fitness, and a program is said to be fitter than another
if it meets the specification on more test cases.

In the setting of program synthesis, genetic programming can be very usefully combined
with the concept of incremental evolution.  The idea is that in order to evolve a program
that is adequate for some difficult task, it is often helpful to first evolve a population
that as an ensemble is good at some simpler variation of the task.  This principle
leads us to the observation that each round of the CEGIS loop can be thought of
as creating a more difficult problem for the GP synthesiser to solve.  Therefore,
if GP finds a candidate program which is correct for the $k$ inputs so far observed,
but is not correct for the full specification, that candidate program came from a population
that was ``good'' for the specification restrict to those $k$ inputs.  The next time round
the CEGIS loop, the problem GP must solve is slightly harder -- any solution to this new
problem must be correct for the previous $k$ inputs and also whatever new input was added
by the previous iteration of {\sc verif}.  It therefore makes sense to begin the $k+1$st
round of evolution with the population at the end of the $k$th round.
...
\section{Experiments}
We implemented our algorithm in the {\sc Headshot} tool, which uses the {\sc Kalashnikov} program synthesiser for termination.
