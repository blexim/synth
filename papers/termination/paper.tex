...
complete procedure for the termination analysis of programs with fixed-width
...
large fragment of C, but is not rich enough to analyse code that uses the
heap or unbounded arrays.  Also, we have not yet added support for recursion
to our encoding.
...
The final source of difference between integer arithmetic and bit-vector
...
States $x$ satisfying the loop's guard are given by the
