...
runtime is bounded by $2 \times n \times NP(n \times l \times \| L \|)$, which is bounded
by $2^{bk + 1} NP(2^{bk} \times l \times \| L \|)$.  Clearly the runtime is affected most strongly by $n$,
...
we will terminate).  So it is clear that the space of solutions we search over is the space
of functions computed by $\mathcal{L}$-programs of length no greater than $K(\sigma)$.  Let's
denote this set $\mathcal{L}(K(\sigma))$.  Clearly, $\| \mathcal{L}(K(\sigma)) \| \leq O(2^{K(\sigma)})$
since there are $O(2^{K(\sigma)}$ \emph{programs} of length $K(\sigma)$ and some functions
will be computed by more than one program.

Each iteration of the CEGIS loop distinguishes at least one incorrect function from the set of correct
functions, so we can see that the loop will iterate no more than $\| \mathcal{L}(K(\sigma)) \|$ times.
...
Conversely, the probability that a random program of length $l$ computes
a linear function is $O(2^{-l})$, since otherwise we'd need to have an exponential
number of programs computing the same function.
