...
For our case, given a loop $L = \langle I,G,T \rangle$, the notion of open recurrent set is encoded by formula {\bf [ONT]}. %Definition~\ref{def:nonterm-formula}.
...
it terminates.  We will now consider the problem of encoding the loop's environment as
a synthesis formula.
...
The loop we consider now is $L=\langle I,G,T \rangle$; it has an additional predicate $I(x)$ 
...
\subsection{Genetic Programming and Incremental Evolution}
