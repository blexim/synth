...
our formula that encode the fact that $T_o$ is an overapproximation of $T$, and that it is
...
example in Figure~\ref{fig:environment-model}.  Our technique proves the
termination of each loop separately and in program order, with the exception
of nested loops, whose termination we encode in the same formula.  Following
the same design, let us assume that we have already proven the termination
of loop $L_1$, and we are now trying to prove that $L_2$ and $L_3$
terminate, which we encode as the conjunction of constraints on the
...
Note that despite its intricacy, the constraint system is still a formula in
the synthesis fragment.  Since the outer loop of this program does indeed
terminate, the system is satisfiable, as witnessed by:
%
\begin{align*}
...
Since $L$ either terminates or does not terminate, this formula is a
tautology in the synthesis fragment.  A solution to the formula would
...
\begin{proof}
A very inefficient construction which computes $f$ is:
%
\begin{verbatim}
t1 = f(0)
t2 = v1 == 1
...
\end{verbatim}
%
Thus, every $f$ is computed a program of size at most $b \times
2^{bk + 1}$.
...
\begin{proof}
 ...
 a successor for each $x \in N$.  Since $\mathcal{S}$ is finite, and by Lemma~\ref{lem:l-func}, both
...
\begin{proof}
 ...
 in $\mathcal{L}$.  Since $\phi$ is a termination specification, $P_T \models \phi$ and so $P_T \models \phi \vee \psi$.

 Similarly if $L$ does not terminate for some input, by Theorem~\ref{thm:l-nonterm} there is a non-termination
 proof $P_N$ expressible in $\mathcal{L}$, so $P_N \models \psi$ and so $P_N \models \phi \vee \psi$. 
\end{proof}

\begin{theorem}
 Algorithm~\ref{alg:cegis} terminates on the generalised termination specification of Definition~\ref{def:general-termination}.
\end{theorem}

\begin{proof}
 The specification is satisfiable (Thoerem~\ref{thm:generalised-sat}) and Algorithm~\ref{alg:cegis} is semi-complete
 (Theorem~\ref{thm:synth-semicomplete}).
\end{proof}

Hence our procedure is sound and complete for bitvector termination.

{\bf TODO:} Tie everything up into one algorithm (build the formula, invoke CEGIS, check which witness is
correct) then proof soundness for that.
