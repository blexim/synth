...
%% deciding termination of a loop $L$.  This is given as Algorithm~\ref{alg:termination}.
...
%% is first to prove that Algorithm~\ref{alg:cegis} (as instantiated
...
%%  Algorithm~\ref{alg:cegis} does as well.
...
%% We will now show that $\mathcal{L}$ is expressive enough to encode
%% \mbox{(non-)}ter\-mi\-nation proofs for every bit-vector program.
...
\begin{theorem}
Our technique %Algorithm~\ref{alg:termination} 
...
%% of the Kolmogorov complexity of the \mbox{(non-)}ter\-mi\-nation proof, and so
...
SV-COMP'15~\cite{svcomp15}\footnote{The logs and benchmarks are included in the
non-anonymous supplementary material submitted with this paper}.%\footnote{The benchmarks, code and log files may be downloaded from the URL \texttt{http://www.cprover.org/juggernaut/}}.
To
...
It solves 4 of the 5 nested loops in less than 30\,s\footnote{Although
