...
The resulting technique is a sound and complete analysis for the termination 
...
%% In our setting this type of programs does not need special treatment, as
For program (i) we find a nonlinear lexicographic ranking function $R(x, y, z) = (x < y,
...
decrease indefinitely, and so $R$ proves that the loop is well founded.}
 We are not aware of any linear ranking function for this program.
...
$Q_1 x_1 \ldots Q_n x_n . \sigma$ is the special case of
a propositional formula with first-order quantification,
i.e.~QBF.
\end{definition}
...
the value currently stored in \lstinline|x|.  Thus, this loop
...
\begin{proof}
 ...
 a successor for each $x \in N$.  The state space $\mathcal{S}$ is finite, so by Lemma~\ref{lem:l-func} both
