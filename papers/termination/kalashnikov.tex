...
\node [file] (spec) {\sc Specification};
...
\path (spec.east)+(2.0, 1.5) node [process] (cbmc) {\sc cbmc};
\path (spec.east)+(2.0, 0.0) node [process] (gcc) {\sc gcc};
\path (spec.east)+(2.0, -1.5) node [process] (gp) {\sc GP};

\path (gcc.east)+(2.5, 0.0) node [file] (out) {Proof};
