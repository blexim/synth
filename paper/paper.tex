...
which is also the closest work to ours is.  {\sc brahma} is also a method
for synthesizing loop-free bitvector programs.  In~\cite{brahma}, the
...
sketching}, in particular the {\sc sketch} tool~\cite{sketch}.  With this
method the user supplies a program sketch, written in a C-like language,
together with either a specification (written in the same language) or a
suite of unit tests.  The sketch is a skeleton program which contains much
of the high level structure of the final program, but with several ``holes''
that {\sc sketch} fills in to meet the specification, again using the CEGIS
algorithm.
