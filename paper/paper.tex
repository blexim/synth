...
Finally, column 8 shows the runtime for {\sc Kalashnikov} and column 9 the number of instructions
in the program synthesised by {\sc Kalashnikov}.  The results can be divided into
...
any of the tools was significantly faster than the others, so we performed a Mann-Whitney U test.
Since the {\sc brahma} experiments were performed on a slower machine than the
{\sc kalashnikov} ones, we multiplied each of the runtimes by the CPU frequency of the machine
that was used for that experiment.  This gives an estimate of the time we would expect the test to
have taken on a 1GHz machine.  Scaling in this way  is more or less legitimate, since we have empirically
observed that our algorithm is CPU bound.

For each pair of tools ({\sc kalashnikov} vs. each of the {\sc brahma} configurations), the U test was
unable to reject the null hypothesis (that the tools are equally fast) at the p=0.05 level. In other words,
there is no statistically significant difference in the speed of {\sc kalashnikov} and {\sc brahma} on this
set of benchmarks, even after correcting for the difference in CPU frequency.
