...
fragment as C as a ``high level'' logic.
...
and arbitrary-width types.  Two example \newC programs are shown 
in Fig.~\ref{fig:c-}.
...
The exact details of how we encoding an $\mathcal{L}$-program are given in Sec.~\ref{sec:encode-l}.
...
The \verb|prog_t| structure encodes a complete program, which is a sequence of instructions.
...
a program argument or the result of a previous instruction.  The value of an operand
...
no operand refers to the result of an instruction that has not been computed yet.
\label{defn:well-formed}
\end{defn}

We add a well-formedness constraint of the form \verb|params[i] <= (a + c + 2*i)|
...
high-complexity language.  Our parametrisation is constructed
...
If {\sc synth} succeeds by {\sc verif} fails, we have a candidate
...
{\sc CBMC}~\cite{cbmc-website} at SVN revision r2777, with Glucose 2.2~\cite{glucose-paper} as the SAT solver.
...
on the same benchmarks, as reported in~\cite{brahma} and~\cite{brahma-icse}\footnote{Unfortunately
we were not able to obtain a copy of {\sc Brahma}, and so were unable to rerun the experiments ourselves.}.
These experiments were performed an 8-core,
1.86\,GHz Xeon with 4\,GB of RAM.  Although there are 49 results shown across the two tables, it should
be noted that there is a lot of overlap between the two sets of benchmarks and that there are only
29 unique benchmarks altogether.
...
The tables include the runtime for each tool, the number of lines in the synthesised program and whether
{\sc Brahma} was able to complete the synthesis completely automatically (the ``Aut.''~column).  A blank
in the ``Aut.''~column means that {\sc Brahma} was able to synthesise the program automatically, and a \xmark
\,means that user assistance was required.
...
This represents the majority of cases.  A statistical breakdown of this case is given in Fig~\ref{fig:stats-table}.
{\sc Kalashnikov} performs substantially better than {\sc Brahma} on these tests, and similarly to
semibiased {\sc Brahma} (an optimisation introduced in~\cite{brahma-icse}).  Program synthesis appears to
be CPU-bound, and so part of the performance difference may be explained by the faster CPU in the machine
used to run {\sc Kalashnikov}.
