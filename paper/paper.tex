...
Since then the program synthesis problem for programs has been attacked by Manna
and Waldinger using theorem provers~\cite{manna-waldinger}, and by
Kraan et al. using proof planning techniques~\cite{proof-planning}.
...
Our task is to find a program that satisfies some specification.  We can formalize this
...
\begin{itemize}
 ...
 \verb|void exec(prog_t p, int in[N], int out[M])|.  Here, \verb|out| is an ouput parameter.
\end{itemize}
...
programs from~\cite{brahma} and~\cite{brahma-icse}.  The code we used to perform the experiments, along with the
